...
<?php
    ...
    if($_SESSION["userType"] == 1) {
        ...
        $query = "SELECT investigator.id, investigator.name FROM investigator JOIN user ON investigator.user_id = user.id WHERE investigator.user_id = $id";
                ...
                <div class="modal">
                    <div class="modal-header">
                        <?php
                            ...
                            echo '<h1>Welcome, ' . getFirstAndLast($user["name"]) . '!</h1>';
                        ?>
                    </div>
                    <?php 
                        if ($_SESSION["userType"] == 2) {
                            $query = 'SELECT medic.id, medic.name, count(appointment.medic_id) AS waiting_list 
                                ...
                                WHERE medic.id = ' . $user["id"] . ' AND appointment.prescription IS NULL
                                GROUP BY medic.id';
